...
| Day | Topics Covered |
|-----|----------------|
| Day-008 | Batch API pattern (submit/poll/retrieve); Azure OpenAI Global Batch quota model (separate enqueued-token quota, 50% cost reduction); batch vs. synchronous routing decision; hard-cap budget enforcement at submission boundary; provider abstraction (`IBatchChatModelProvider`) enabling Anthropic ↔ Azure OpenAI swap |
